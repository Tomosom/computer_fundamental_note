...
static bool init_for_each (dll_t *_p_dll, dll_node_t *_p_node, void *_p_arg)
{
    ...
    if (0 != result) {
        console_print ("Error: can't initialize module %s (%s))", p_module->p_name_, errstr (result));
        return false;
    }
    return true;
}
...
static bool down_for_each (dll_t *_p_dll, dll_node_t *_p_node, void *_p_arg)
{
    ...
    if (0 != result) {
         console_print ("Error: can't shut down module \"%s\" (%s)", p_module->p_name_, errstr (result));
        // !!! don't return false
    }
    return true;
}
...
static bool destroy_for_each (dll_t *_p_dll, dll_node_t *_p_node, void *_p_arg)
{
    ...
    if (0 != result) {
         console_print ("Error: can't destroy module \"%s\" (%s)", p_module->p_name_, errstr (result));
        // !!! don't return false
    }
    return true;
}
