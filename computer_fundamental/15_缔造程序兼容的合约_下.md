...
反汇编查看, 可看到函数调用时的细节行为 [frame.s](code/15_缔造程序兼容的合约-下/disassemble/frame.s)
...
# [<u>实验 - 函数调用约定</u>](code/15_缔造程序兼容的合约-下)
...
反汇编查看返回值赋值 [convention.s](code/15_缔造程序兼容的合约-下/disassemble/convention.s)
...
查看上面生成的汇编 [convention.s](code/15_缔造程序兼容的合约-下/disassemble/convention.s)
空函数体编译得到的只有前言和后续
发现不同的调用约定, 前言和后续有所不同
...
# [<u>实验 - 结构体函数返回值</u>](code/15_缔造程序兼容的合约-下)
...
反汇编查看 [return.s](code/15_缔造程序兼容的合约-下/disassemble/return.s)
